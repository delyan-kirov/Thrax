...
future features must compose, so that the small near-term pieces (range syntax
#9, `.n` indexing #10 in doc/syntax-roadmap.md) are built as an on-ramp rather
than a dead end.
...
- "Slices for free" means indexing with a `Range` returns a VIEW, not a copy. So
  it is not a separate feature; it is type-directed indexing overloaded on the
  index type. This unifies with `.n` indexing (#10): `xs.i` (Int) gives an
  element, `xs.r` (Range) gives a slice. One primitive.
...
2. Indexing is one type-directed primitive over the index type: Int gives an
   element, Range gives a slice/view. #9 (ranges) and #10 (`.n`) are two halves
   of it.
...
- Indexing is type-directed on the index type from the start (Int vs Range).
- The array view representation carries `offset` + `shape` + `strides`.
...
- Surface syntax for range-indexing given indexing is `.n`: `m.(i ..= j)`?
  2-D `m.(r, c)`? Needs a spelling.
...
- Buildable now as the on-ramp (only if built with the four "lock now" points):
  #9 range syntax, #10 `.n` indexing.
- Deferred, and to be done together for composition: first-class `Range` type,
  buffer/view/stride tensor representation, type-directed range-indexing, COW,
  and the `data`/`codata` split.
