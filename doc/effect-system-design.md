...
coroutines, exceptions, generators** (and, since we drop lazy data — see §1 —
**codata / streaming**).
...
are NOT lazy data; they are a **separate kind, `@codata`** (see §1a), and
...
- **CAFs are unaffected** — see §7; a lazy-memoized top-level _binding_ is a
...
see §10. `AGTxSUM`'s old lazy-data infinite stream is rewritten as `@codata`
...
**Why affine fits the runtime + RC.** With the reified-K machine (§6) an affine
...
"used" flag; resuming twice is a runtime error (à la OCaml 5
...
Discontinue k : drop k unresumed; unwind its segment running cleanup (finally)
                -- the resume-0 path (exceptions); semantics DEFERRED, see §11
```
...
handler dispatch needs the effect types (§5); the in-place tail case is partly
...
- **Data:** strict + pure => finite DAG ⟹ no cycles => RC complete (§1).
