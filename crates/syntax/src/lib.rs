//! Surface syntax for Thrax: the handle-based AST and a Pratt parser over the
//! [`lexer`].
//!
//! Entry point: [`parse`], which lexes and parses a whole compilation unit into a
//! [`Parsed`] value: the [`Program`] root plus the [`Ast`] stores its handles
//! index into. Nothing borrows the source, so the result is self-owned.
