...
use core::{lower_program, Decls, Interp, Resolved};
...
fn eval_checked(src: &str, name: &str) -> String {
    let parsed = syntax::parse(src).expect("parse");
    let mut checker = types::Checker::new(&parsed.ast);
    checker
...
fn eval_modules(sources: &[&str], name: &str) -> String {
    ...
    for src in sources {
        let (next, program) = syntax::parse_into(ast, src).expect("parse");
        ast = next;
    ...
    for dep in deps {
        let mut c = types::Checker::new(&ast);
        c.check_program(dep).expect("check dep");
        dep_checkers.push(c);
    }
    let mut root_checker = types::Checker::new(&ast);
