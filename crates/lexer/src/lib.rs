...
use arena::Arena;
use diag::{Code, Diagnostic, Line, Result, Span};
...
impl<'a> Lexer<'a> {
    ...
    fn lex_one(&mut self) -> Result<Token<'a>> {
        ...
        match c {
            ...
            _ if token::is_digit(c) => self.lex_number(start, line),
            _ if token::is_ident_start(c) => Ok(self.lex_word(start, line)),
            _ if token::is_operator_char(c) => self.lex_operator(start, line),
            _ => {
                if let Some(kind) = token::delimiter(c) {
                    self.cursor += 1;
    ...
    fn escape_hex_byte(&mut self, start: usize, line: Line, out: &mut Vec<u8>) -> Result<()> {
        ...
        if !token::is_hex_digit(h1) || !token::is_hex_digit(h2) {
            return Err(self.err(
                Code::InvalidEscape,
                start,
                line,
    ...
    fn escape_unicode(&mut self, start: usize, line: Line, out: &mut Vec<u8>) -> Result<()> {
        ...
        let mut cp: u32 = 0;
        let mut digits = 0;
        while token::is_hex_digit(self.cur()) && digits < 6 {
    ...
    fn lex_tyvar(&mut self, start: usize, line: Line) -> Result<Token<'a>> {
        ...
        if !token::is_ident_start(self.cur()) {
            return Err(self.err(
                Code::UnknownSymbol,
                start,
                line,
        ...
        self.scan_while(token::is_ident_cont);
    ...
    fn lex_word(&mut self, start: usize, line: Line) -> Token<'a> {
        self.scan_while(token::is_ident_cont);
        let kind = token::keyword_or_word(self.slice_from(start));
    ...
    fn lex_operator(&mut self, start: usize, line: Line) -> Result<Token<'a>> {
        self.scan_while(token::is_operator_char);
        let lexeme = self.slice_from(start);
        match token::operator(lexeme) {
    ...
    fn lex_number(&mut self, start: usize, line: Line) -> Result<Token<'a>> {
        if self.cur() == b'0' && matches!(self.at(self.cursor + 1), b'x' | b'X') {
            return self.lex_radix(start, line, 16, token::is_hex_digit);
        }
        if self.cur() == b'0' && matches!(self.at(self.cursor + 1), b'b' | b'B') {
            return self.lex_radix(start, line, 2, token::is_bin_digit);
        }

        self.scan_while_digit_or_sep(token::is_digit);

        // A real has a fractional part and/or an exponent.
        let mut is_real = false;
        if self.cur() == b'.' && token::is_digit(self.at(self.cursor + 1)) {
            is_real = true;
            self.cursor += 1; // '.'
            self.scan_while_digit_or_sep(token::is_digit);
        }
        if matches!(self.cur(), b'e' | b'E') {
            ...
            if token::is_digit(self.at(look)) {
                is_real = true;
                self.cursor = look;
                self.scan_while_digit_or_sep(token::is_digit);
            }
