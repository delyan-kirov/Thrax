...
const Operators operator_db{
  ...
  // so the opener may arrive as `<`, the empty row as `<>`, or a label-less
  // open row as `<|`; the closing `>` and the tail separator `|` stand alone.
  // Outside a type these would just be unbound variable names.
  { "<", TokenTag::Op },  //
  { ">", TokenTag::Op },  //
  { "|", TokenTag::Op },  //
  { "<>", TokenTag::Op }, //
