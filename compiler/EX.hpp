...
private:
  ...
  // expression or a pattern. `anchor`/`line` locate the `[`/`::` for diagnostics.
  UT_NODISCARD Expr    *mk_cons(Expr *head, Expr *tail, UT::Vu anchor, size_t line);
  UT_NODISCARD Expr    *mk_nil(UT::Vu anchor, size_t line);
  UT_NODISCARD Pattern *mk_cons_pat(Pattern *h, Pattern *t, UT::Vu a, size_t ln);
