 ...
 *      they are plain Var/App nodes, so no layer below shares an "operator type".
 *
 * What the layers genuinely need to agree on are a handful of *strings*:
 *   - canonical names  ("+", "neg", "if", ...)  -- LX lexes them, EX desugars an
 *                                                  operator to a Var of this name,
 *                                                  TC keys its overload table here.
 *   - type names       ("Int", "Real")          -- how TC spells operand types.
 *   - the `mono` key    "+@Int"                  -- the contract between TC (which
 *                                                  resolves an overloaded use to it)
 *                                                  and IT (which dispatches it).
 *
...
// For binary operators the spelling is also the source lexeme; the unary NEG and
// NOT are distinct names (their lexemes are SUB and BANG) so unary '-' never
// collides with binary '-'.
