...
struct Lowerer
{
  ...
  // Copy a std::string into the arena as a UT::String so it outlives this pass.
  UT::String
  ...
  Expr *
  mk_field(Expr *record, UT::String field)
  {
  ...
  Expr *
  mk_let(UT::String var, Expr *val, Expr *body, EX::Ty *sig)
  {
    EX::ExLet lt;
    lt.var  = var;
    lt.val  = val;
  ...
  Expr *
  mk_if(Expr *cond, Expr *then, Expr *alt)
  {
    Expr e{ ExprTag::If };
  ...
  Expr *
  mk_binop(const char *op, Expr *lhs, Expr *rhs)
  {
    Expr v{ ExprTag::Var };
    v.as     = EX::ExVar{ UT::String{ op, std::strlen(op) } };
    Expr *fn = alloc(v);
  ...
  UT::String
  fresh(const char *hint)
  {
    return ustr("$" + std::string(hint) + std::to_string(fresh_n++));
  }

  void
  err(ER::Code code, UT::String anchor, size_t line, const std::string &msg)
  {
  ...
  bool
  resolve_fields(EX::PatStruct &ps, std::vector<Pattern *> &subs)
  {
    std::string tname = std::to_string(ps.type_name);
    ...
    for (size_t i = 0; i < ps.fields.m_len; ++i)
      if (ps.fields[i].name.m_len) { named = true; break; }

    if (named)
    {
      for (size_t i = 0; i < ps.fields.m_len; ++i)
      {
        std::string fn  = std::to_string(ps.fields[i].name);
  ...
  // (already-lowered) expression `subject`. `anchor`/`line` locate the enclosing
  // pattern for diagnostics. When `refutable_ok` is false (lambda / let), a
  // literal sub-pattern is an error; when true (match arm), its equality test is
  // emitted separately by `test_of`, so here it only contributes no binding.
  Expr *
  bind_pattern(Pattern *pat, Expr *subject, Expr *body, UT::String anchor,
               size_t line, bool refutable_ok)
  {
    switch (pat->tag)
    {
  ...
  Expr *
  bind_struct(Pattern *pat, Expr *subject, Expr *body, UT::String anchor,
              size_t line, bool refutable_ok)
  {
    ...
    auto it = structs.find(std::to_string(ps.type_name));
    const std::vector<std::string> &decl = it->second;
  ...
  // expression of statically known type). Returns null when `pat` is irrefutable
  // (always matches). Only the literal/struct-literal parts contribute a test;
  // variable/wildcard parts are bound separately by `bind_pattern`.
  Expr *
  test_of(Pattern *pat, Expr *subject)
  {
    switch (pat->tag)
    {
    case PatTag::Wild:
    ...
    case PatTag::Struct:
    {
      ...
      const std::vector<std::string> &decl
        = structs.find(std::to_string(ps.type_name))->second;
      Expr *acc = nullptr;
  ...
  // of the scrutinee followed by a chain of boolean ifs whose conditions are the
  // arms' equality tests and whose then-branches bind the arm's variables. An
  // irrefutable arm (no test) always matches, ending the chain.
  Expr *
  lower_match(Expr *e)
  {
    ...
    UT::String mv    = fresh("m");
    Expr      *mvar  = mk_var(mv);
    Expr      *chain = alt;
    for (size_t i = m.arms.m_len; i-- > 0;)
    {
  ...
  // place; pattern lambdas/lets are replaced by their desugared equivalents.
  Expr *
  lower(Expr *e)
  {
    switch (e->tag)
    ...
    case ExprTag::Let:
    {
      ...
      if (lt.pat)
      {
        ...
        return bind_pattern(lt.pat, val, body, anc, line, /*refutable_ok=*/false);
      }
      lt.val  = lower(lt.val);
  ...
  static void
  pat_anchor(Pattern *pat, UT::String &anchor, size_t &line)
  {
    switch (pat->tag)
    {
    case PatTag::Struct:
  ...
  run(EX::Exprs &exprs)
  {
    ...
    for (size_t i = 0; i < exprs.m_len; ++i)
    {
      ...
      for (size_t k = 0; k < sd.fields.m_len; ++k)
        fns.push_back(std::to_string(sd.fields[k].name));
      structs[std::to_string(sd.name)] = std::move(fns);
    }
